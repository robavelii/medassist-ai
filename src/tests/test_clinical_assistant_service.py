...
def mock_llm_cache_service():
    ...
    service.get_latest_cached_response_by_patient_and_model = MagicMock(
        return_value=None
    )
...
class TestClinicalAssistantServiceCachedResponses:
    ...
    @pytest.mark.asyncio
    async def test_get_cached_response_returns_none_when_no_cache(
        self, clinical_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_cached_response_handles_json_decode_error(
        self, clinical_service, mock_llm_cache_service
    ):
        mock_entry = MagicMock()
        mock_entry.response = "invalid-json"
        mock_entry.patient_id = "test-patient-001"
        mock_entry.visit_id = "test-visit-001"
        mock_llm_cache_service.get_latest_cached_response_by_patient_and_model.return_value = (
            mock_entry
        )
...
class TestClinicalAssistantServiceLLMResponses:
    ...
    @pytest.mark.asyncio
    async def test_triage_cold_cases_calls_llm(
        self, clinical_service, mock_llm_cache_service, sample_vitals
    ):
        ...
        input_data = TriageColdCaseInput(
            ...
            patient_info=BaseInput(
                patient_id="test-patient-001", gender="male", age=35
            ),
    ...
    @pytest.mark.asyncio
    async def test_emergency_triage_calls_llm(
        self, clinical_service, mock_llm_cache_service, sample_vitals
    ):
        mock_response = json.dumps(
            {
...
class TestCostEstimation:
    ...
    async def test_estimate_costs_scales_with_patients(self, clinical_service):
        ...
        assert (
            result_5.total_estimated_input_tokens
            == result_1.total_estimated_input_tokens * 5
        )
